...
def edit_suballoc(id):
    ...
    return render_template(
        "admin/adm_edit_suballoc.html", data=ce, data2=se, data3=co
    )
...
def cmp_add_shortlist_post():
    ...
    if btn == "SEARCH":
        ...
        for id in ids:
            qry = c.nonreturn(
                "update requests set status='approved' where r_id='" + id + "'"
            )
            qry2 = c.selectone(
                "select v_id,s_id from requests where r_id='" + id + "'"
            )
            qry3 = c.nonreturn(
...
def cmp_remove_shortlist(rid):
    c = conn()
    dt = c.nonreturn(
        "update requests set status='not-selected' where r_id='" + rid + "'"
    )
    qry2 = c.selectone(
        "select v_id,s_id from requests where r_id='" + rid + "'"
    )
    qry3 = c.nonreturn(
...
def verify():
    ...
    if res is not None:
        ...
        img_url = r"D:\web\SecureID" + res[0]
