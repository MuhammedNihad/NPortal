...
def college_studymaterialsupload() -> str:
...
def call_adm_ajax_course_call_internalintern() -> Response:
    ci = conn()
    ...
    session["clg"] = clg
    de = (
        "select course.c_id,course.c_name from course inner join course_allocation on course.c_id=course_allocation.c_id where course.d_id='"
        + dep
        + "' and course_allocation.col_id='"
        + clg
        + "'"
    )
    res = ci.jsonsel(de)
...
def adm_ajax_studdd() -> Response:
    ci = conn()
    clg = session["clg"]
    dep = session["dep"]
    cou = request.form["cou"]
    de = (
        "select student.*,department.d_name,course.c_name,college.col_name from student,college,course,department where student.col_id=college.login_id and student.c_id=course.c_id and student.d_id=department.d_id and student.col_id='"
        + clg
        + "' and student.c_id='"
        + cou
        + "' and student.d_id='"
...
def call_intern() -> str:
    ...
    ka = (
        "insert into internal_requests values(null,'"
        + clg
        + "','"
        + dep
        + "','"
...
def call_adm_ajax_course_view_internal() -> Response:
    ci = conn()
    clg = request.form["clgid"]
    dep = request.form["dptid"]
    de = (
...
def adm_internal() -> str:
    clg = request.form["clg"]
    dep = request.form["dpt"]
    sem = request.form["sem"]
    session["sem"] = sem
    se = (
        "select student.*,department.d_name,college.col_name from college,department,student where college.login_id=student.col_id and student.d_id=department.d_id and student.col_id='"
        + clg
        + "' and student.d_id='"
        + dep
        + "' and student.sem='"
...
def adm_view_inte(stid: int) -> str:
    if session["logout"] == "1":
        qry = (
            "select subject.sub_code,subject.sub,internal.internals from internal,subject where internal.sub_id=subject.sub_id and subject.sem='"
            + str(session["sem"])
            + "' and internal.s_id='"
            + str(session["st_id"])
            + "'"
...
def call_adm_course_view_internal() -> Response:
    ci = conn()
    sem = request.form["sem"]
    cou = request.form["cou"]
    de = (
        "select sub_id,sub from subject where c_id='"
        + cou
        + "' and sem='"
        + sem
        + "'"
...
@app.route("/reply/<id>")
def reply(id: int) -> str:
    if session["logout"] == "1":
        c = conn()
        de = "select * from complaint where cm_id='" + str(id) + "'"
        ce = c.selectone(de)
...
def comp_reply() -> str:
    ...
    up = (
        "update complaint set reply='"
        + rep
        + "',status='replied' where cm_id='"
        + com
        + "'"
    )
    c.nonreturn(up)
...
def adm_cert_gen(stid: int) -> str:
    ...
    url.svg(static_path + "qr\\" + str(stid) + ".svg", scale=8)

    qry = (
        "select department.d_name,student.s_name,course.c_name from department,course,student where student.c_id=course.c_id and student.d_id=department.d_id and student.login_id='"
        + str(stid)
        + "'"
    )
    res = c.selectone(qry)
    qr_path = "/static/qr/" + str(stid) + ".svg"
...
def course_by_clg() -> Response:
    c = conn()
    clg = request.form["clg"]
    session["col_id"] = clg
    zz = (
        "select * from course where c_id not in(select c_id from course_allocation where col_id='"
        + clg
        + "')"
    )
    res = c.jsonsel(zz)
...
def cou_allo() -> str:
    ...
    cou = request.form["cou"]
    qry = (
        "SELECT * FROM course_allocation WHERE col_id='"
        + clg
        + "' AND c_id='"
        + cou
        + "'"
    ...
    if res is not None:
        ...
        a = (
            "insert into course_allocation values(null,'"
            + clg
            + "','"
            + cou
            + "')"
...
def search_course() -> str:
    if session["logout"] == "1":
        c = conn()
        txt = request.form["ser"]
        co = (
            "select college.col_name,course.c_name,course_allocation.col_id,course_allocation.ca_id from course,college,course_allocation where college.login_id=course_allocation.col_id and course.c_id=course_allocation.c_id and college.col_name like '%"
            + txt
            + "%' "
        )
        cu = c.selectall(co)
...
def del_coualc(id: int) -> str:
    c = conn()
    de = (
        "delete from course_allocation where course_allocation.ca_id='"
        + str(id)
        + "'"
    )
    c.nonreturn(de)
...
def edit_cou(id: int) -> str:
    c = conn()
    d = "select * from department"
    r = c.selectall(d)
    qr = (
        "select course.c_id,course.c_name,department.d_id,department.d_name from course,department where course.d_id=department.d_id and course.c_id='"
        + str(id)
        + "'"
    )
    dd = c.selectone(qr)
...
def delete_cou(id: int) -> str:
    de = "delete from course where c_id='" + str(id) + "'"
    c = conn()
    c.nonreturn(de)
    return view_cou()
...
def edit_dep(id: int) -> str:
    c = conn()
    q = "select * from department where d_id='" + str(id) + "'"
    r = c.selectone(q)
...
def delete_dpt(id: int) -> str:
    d = "delete from department where d_id='" + str(id) + "'"
    c = conn()
    c.nonreturn(d)
    return view_dpt()
...
def adm_sub_alloc() -> str:
    ca = conn()
    cou = request.form["cou"]
    sub = request.form["sub"]
    se = request.form["sem"]
    al = (
        "insert into subject_allocation(subal_id,c_id,sub_id,sem) values (null,'"
        + cou
        + "','"
        + sub
        + "','"
...
def edit_suballoc(id: int) -> str:
    c = conn()
    co = (
        "select course.c_name,subject.sub,subject_allocation.sem,subject_allocation.c_id,subject_allocation.subal_id,subject_allocation.sub_id,course.c_id from course,subject_allocation,subject where subject_allocation.sub_id=subject.sub_id and course.c_id=subject_allocation.c_id and subject_allocation.subal_id='"
        + str(id)
        + "'"
    )
    ce = c.selectone(co)
    ...
    all_subjects = c.selectall(select_all_subject)
    return render_template(
        "admin/adm_edit_suballoc.html", data=ce, data2=se, data3=all_subjects
    )


@app.route("/update_suballoc", methods=["post"])
def update_suballoc() -> str:
    cu = request.form["cou"]
    si = request.form["sub"]
    se = request.form["sem"]
    ...
    c = conn()
    de = (
        "update subject_allocation set c_id='"
        + cu
        + "',sub_id='"
        + si
        + "',sem='"
...
def del_suballo(id: int) -> str:
    c = conn()
    de = (
        "delete from subject_allocation where subject_allocation.subal_id='"
        + str(id)
        + "'"
    )
    c.nonreturn(de)
...
def adm_sub_mngmnt_post() -> str:
    ...
    ca.nonreturn(co)
    return (
        "<script>alert('Subject Added');window.location='/sub_mgmt';</script>"
    )
...
def view_clg() -> str:
    if session["logout"] == "1":
        c = conn()
        bb = (
            "select * from college,login where college.login_id=login.login_id"
        )
        r = c.selectall(bb)
...
def view_clg_post() -> str:
    name = request.form["name"]

    if session["logout"] == "1":
        c = conn()
        bb = (
            "select * from college,login where college.login_id=login.login_id and col_name like '%"
            + name
            + "%'"
        )
        r = c.selectall(bb)
...
def viewmore_clg(logid: int) -> str:
    c = conn()
    bb = (
        "select * from college,login where college.login_id=login.login_id and login.login_id='"
        + str(logid)
        + "' "
    )
    r = c.selectone(bb)
...
def pubviewmore_clg(logid: int) -> str:
    c = conn()
    bb = (
        "select * from college,login where college.login_id=login.login_id and login.login_id='"
        + str(logid)
    ...
    r = c.selectone(bb)
    bb = (
        "SELECT `course`.* FROM `course_allocation`,`course` WHERE `course_allocation`.`c_id`=`course`.`c_id` AND `course_allocation`.`col_id`='"
        + str(logid)
        + "' "
    )
    res = c.selectall(bb)
...
def adm_acc_rej_clg() -> str:
    ...
    if btn == "ACCEPT":
        ...
        c.nonreturn(qry)
        return (
            "<script>alert('Accepted');window.location='/view_clg';</script>"
        )
    else:
        qry = "update login set type='reject' where login_id='" + logid + "'"
        c.nonreturn(qry)
        return (
            "<script>alert('Rejected');window.location='/view_clg';</script>"
        )
...
def dept_wise() -> str:
    ...
    cc = (
        "select department.d_name,course.* from course,department where department.d_id=course.d_id AND department.d_id='"
        + names
        + "'"
    )
    ca = c.selectall(cc)
...
def view_subtsby() -> str:
    ...
    res1 = c.selectall(bb)
    aa = (
        "select course.c_name,subject.sub,subject.sub_code,subject.sub_id,subject.sem from subject,course where course.c_id=subject.c_id AND subject.c_id='"
        + name1
        + "' AND subject.sem='"
        + name2
        + "'"
...
def edit_sub(id: int) -> str:
    c = conn()
    co = "select * from course"
    su = (
        "select course.c_id,course.c_name,subject.sub_id,subject.sub,subject.sub_code,subject.sem from course,subject where subject.c_id=course.c_id and subject.sub_id='"
        + str(id)
        + "'  "
    )
    ab = c.selectall(co)
...
def delete_sub(id: int) -> str:
    c = conn()
    da = "delete from subject where sub_id='" + str(id) + "'"
    c.nonreturn(da)
    return view_sub()
...
def adm_add_ext() -> str:
    ...
    session["sem"] = sem
    zz = (
        "select student.photo,student.s_name,course.c_name,department.d_name,college.col_name,course.c_id,student.login_id from student,course,department,college where student.col_id='"
        + clg
        + "' and student.d_id='"
        + dep
        + "' and student.c_id='"
...
def ent_ext(stid: int) -> str:
    if session["logout"] == "1":
        c = conn()
        session["st_id"] = stid
        so = (
            "select * from subject where subject.c_id='"
            + str(session["crs_id"])
            + "'and subject.sem='"
            + str(session["sem"])
            + "' "
...
def adm_enter_ext() -> str:
    ca = conn()
    sub = request.form["sub"]
    mark = request.form["mark"]
    qr = ca.selectone(
        "select * from external where s_id='"
        + str(session["st_id"])
        + "' and sub_id='"
        + sub
        + "'"
    ...
    if qr is None:
        re = (
            "insert into external values(null,'"
            + str(session["st_id"])
            + "','"
            + sub
            + "','"
...
def collegedel(cid: int, id: int) -> str:
    d = conn()
    d.nonreturn("delete from login where login_id='" + str(id) + "'")
    d.nonreturn("delete from college where col_id='" + str(cid) + "'")
...
def stf_mngmnt() -> str:
    if session["logout"] == "1":
        c = conn()
        de = (
            "select distinct(department.d_id),d_name from department,course,course_allocation where course.c_id=course_allocation.c_id and department.d_id=course.d_id and course_allocation.col_id='"
            + str(session["clg_id"])
            + "'"
        )
        do = c.selectall(de)
...
def clg_view_staff() -> str:
    if session["logout"] == "1":
        c = conn()
        st = (
            "select staff.*,department.d_name from department,staff where staff.dep_name=department.d_id and staff.col_id='"
            + str(session["clg_id"])
            + "'"
        )
        sa = c.selectall(st)
...
def clg_view_staff_search_post() -> str:
    name = request.form["name"]
    if session["logout"] == "1":
        c = conn()
        st = (
...
def update_stf(id: int) -> str:
    c = conn()
    st = (
        "select staff.*,department.d_name from department,staff where staff.dep_name=department.d_id and staff.staf_id='"
        + str(id)
        + "'"
    )
    sa = c.selectone(st)
...
def delete_stf(id: int) -> str:
    c = conn()
    se = "delete from staff where staf_id='" + str(id) + "'"
    c.nonreturn(se)
    return clg_view_staff()


@app.route("/clg_view_cou")
def clg_view_cou() -> str:
    if session["logout"] == "1":
        c = conn()
        de = (
            "select distinct(department.d_id),d_name from department,course,course_allocation where course.c_id=course_allocation.c_id and department.d_id=course.d_id and course_allocation.col_id='"
...
def clg_cou() -> str:
    ...
    cc = (
        "select course.c_id,course.c_name from course,course_allocation where course.c_id=course_allocation.c_id and course_allocation.col_id='"
        + session["clg_id"]
        + "' and course.d_id='"
        + dept
        + "'"
...
def view_subjects(id: int) -> str:
    c = conn()
    qry = "SELECT * FROM SUBJECT WHERE c_id='" + str(id) + "'"
    res = c.selectall(qry)
    return render_template("College/clg_sub_view.html", data=res)


@app.route("/clg_view_profile")
def clg_view_profile() -> str:
    if session["logout"] == "1":
        c = conn()
...
def clg_view_request() -> str:
    if session["logout"] == "1":
        c = conn()
        pr = "select * from course"
        ac = c.selectall(pr)
        return render_template(
            "College/clg_view_internalmark_request.html", data2=ac
        )
    else:
        return login()
...
def clg_internal() -> str:
    c = conn()
    dept = request.form["cou"]
    x = session["clg_id"]
    cc = (
        "select internal_requests.*,course.c_name from internal_requests,course where course.c_id=internal_requests.crs_id and internal_requests.crs_id='"
        + dept
        + "' and internal_requests.col_id='"
        + str(x)
        + "'"
    )
    ce = c.selectall(cc)
    return render_template(
        "College/clg_view_internalmark_request.html", data=ce
    )


@app.route("/update_request/<rid>")
def update_request(rid: int) -> str:
    c = conn()
    ut = (
        "update internal_requests set status='confirmed' where ir_id='"
        + str(rid)
        + "'"
    )
    c.nonreturn(ut)
...
def clg_std_mng() -> str:
    if session["logout"] == "1":
        c = conn()
        co = (
            "select distinct(department.d_id),department.d_name from course_allocation,course,department where course_allocation.c_id=course.c_id and course.d_id=department.d_id and course_allocation.col_id='"
            + str(session["clg_id"])
            + "'"
        )
        cu = c.selectall(co)
        return render_template(
            "College/clg_student_registration.html", data=cu
        )
    else:
        return login()
...
def clg_crs_by_dep_ajax() -> Response:
    dep = request.form["dep"]
    clg = str(session["clg_id"])
    qr = (
        "select distinct(course.c_name),course.c_id from course_allocation,course where course_allocation.c_id=course.c_id and course.d_id='"
        + dep
        + "' and course_allocation.col_id='"
        + clg
        + "'"
...
def std_reg() -> str:
    ...
    if res1 is not None:
        ...
        lid = c.nonreturn(qr)
        st = (
            "insert into student(col_id,c_id,s_name,gender,h_name,post,pin,email,photo,login_id,sem,batch,d_id,phone) values('"
            + str(colid)
            + "','"
            + crs
            + "','"
...
def view_std() -> str:
    if session["logout"] == "1":
        c = conn()
        st = (
            "select * from student where col_id='"
            + str(session["clg_id"])
            + "' "
        )
        se = c.selectall(st)
        ...
        b = c.selectone(a)
        return render_template(
            "College/clg_view_student.html", data=se, data2=b
        )
    else:
        return login()
...
def edit_std(id: int) -> str:
    c = conn()
    sa = "select * from student where s_id='" + str(id) + "'"
    si = c.selectone(sa)
    co = (
        "select distinct(department.d_id),department.d_name from course_allocation,course,department where course_allocation.c_id=course.c_id and course.d_id=department.d_id and course_allocation.col_id='"
        + str(session["clg_id"])
    ...
    cu = c.selectall(co)
    return render_template(
        "College/clg_Update_student.html", data=si, data2=cu
    )
...
def delete_std(id: int, did: int) -> str:
    c = conn()
    ds = "delete from student where s_id='" + str(id) + "'"
    c.nonreturn(ds)
    qry = "delete from login where login_id='" + str(did) + "'"
    c.nonreturn(qry)
    return view_std()


@app.route("/placemnt_managmnt")
def placemntcell_reg() -> str:
    if session["logout"] == "1":
...
def placement_reg() -> str:
    ...
    st = (
        "insert into placement values(null,'"
        + stf
        + "','"
        + gen
        + "','"
...
def view_cell() -> str:
    if session["logout"] == "1":
        c = conn()
        sc = (
            "select * from placement where col_id='"
            + str(session["clg_id"])
            + "'"
        )
        se = c.selectall(sc)
...
def collegeviewsubjects_post() -> str:
    d = conn()
    qry = "SELECT * FROM `course`"
    crs = d.selectall(qry)
    ...
    qry = (
        "SELECT * FROM `subject` WHERE `c_id`='"
        + cid
        + "' AND `sem`='"
        + sem
        + "'"
    ...
    res = d.selectall(qry)
    return render_template(
        "/College/Collegeviewsubjects.html", crs=crs, ressubjects=res
    )


@app.route("/collegestudymaterialsload/<subid>")
def collegestudymaterialsload(subid: int) -> str:
...
def college_studymaterialsupload_post() -> str:
    ...
    files.save(
        "D:\\Aproject\\NEWS PORTAL\\web\\SecureID\\SecureID\\static\\Studymaterials\\"
        + timestr
        + ".jpg"
    )
    ...
    qry = (
        "INSERT INTO `studymaterials` (`subid`,`title`,`filename`,`fdate`,`clgid`) VALUES ('"
        + subid
        + "','"
        + title
        + "','"
...
def college_studymaterialsupload_view() -> str:
    subid = session["selsubid"]
...
def update_placement() -> str:
    ...
    if "photo" in request.files:
        ...
        if im.filename == "":
            st = (
                "update placement set phead_name='"
                + stf
                + "',gender='"
                + gen
                + "',h_name='"
...
def delete_placement(id: int) -> str:
    c = conn()
    da = "delete from placement where p_id='" + str(id) + "'"
    c.nonreturn(da)
    return view_cell()


@app.route("/clg_view_std_dept")
def view_std_dpt() -> str:
    if session["logout"] == "1":
        c = conn()
        co = (
            "select distinct(department.d_id),department.d_name from course_allocation,course,department where course_allocation.c_id=course.c_id and course.d_id=department.d_id and course_allocation.col_id='"
...
def view_view_std() -> str:
    ...
    st = (
        "select s_name,course.c_name,gender,email,phone,photo,login_id from student,course where student.c_id='"
        + cou
        + "' and sem='"
        + sem
        + "' and student.d_id='"
        + did
        + "' and course.c_id=student.c_id"
    )
    sd = c.selectall(st)
    co = (
        "select distinct(department.d_id),department.d_name from course_allocation,course,department where course_allocation.c_id=course.c_id and course.d_id=department.d_id and course_allocation.col_id='"
    ...
    cu = c.selectall(co)
    return render_template(
        "College/clg_view std_department.html", data3=sd, data=cu
    )


@app.route("/clg_std_view_internal/<sid>")
def clg_std_view_internal(sid: int) -> str:
    if session["logout"] == "1":
        c = conn()
        sem = session["selsemid"]
        ma = (
            "select subject.sub,sub_code,internals from subject,internal where internal.s_id='"
            + str(sid)
            + "' and subject.sem='"
            + sem
            + "' and subject.sub_id=internal.sub_id"
        )
        me = c.selectall(ma)
...
def clg_std_view_external(sid: int) -> str:
    c = conn()
    sem = session["selsemid"]
    me = (
        "select subject.sub,sub_code,external from subject,external where external.s_id='"
        + str(sid)
        + "' and subject.sem='"
        + sem
        + "' and subject.sub_id=external.sub_id"
    )
    ma = c.selectall(me)
...
def stf_view_profile() -> str:
    if session["logout"] == "1":
        c = conn()
        sf = (
            " select staff.*,department.d_name from staff,department where staff.dep_name=department.d_id and staff.login_id='"
            + session["stf_id"]
            + "'"
        )
        st = c.selectall(sf)
...
def stf_view_std() -> str:
    if session["logout"] == "1":
        ...
        ce = (
            "select course.c_name,course.c_id from course,staff where course.d_id=staff.dep_name and staff.login_id='"
            + session["stf_id"]
            + "'"
        )
        co = c.selectall(ce)
...
def stf_view_stu() -> str:
    c = conn()
    co = request.form["cou"]
    se = request.form["sem"]
    session["sem"] = se
    ss = (
        "select student.s_name,course.c_name,student.login_id from student,course where student.c_id='"
        + co
        + "' and student.sem='"
        + se
        + "' and student.c_id=course.c_id"
    )
    st = c.selectall(ss)
...
def stf_add_int(sid: int) -> str:
    c = conn()
    su = (
        "select subject.sub,subject.sub_id from student,subject where student.c_id=subject.c_id and student.login_id='"
        + str(sid)
        + "' and subject.sem='"
        + session["sem"]
        + "'"
...
def add_mark() -> str:
    ...
    c = conn()
    qr = c.selectone(
        "select * from internal where s_id='"
        + st_id
        + "' and sub_id='"
        + sub
        + "'"
    ...
    if qr is None:
        it = (
            "insert into internal values(null,'"
            + st_id
            + "','"
            + sub
            + "','"
...
def stf_view_ext() -> str:
    c = conn()
    co = request.form["cou"]
    se = request.form["sem"]
    ss = (
        "select student.s_name,course.c_name,subject.sub,subject.sub_code,external.mark from student,course,external,subject where student.c_id='"
        + co
        + "' and student.sem='"
        + se
        + "' and student.c_id=course.c_id and student.s_id=external.s_id and external.sub_id=subject.sub_id"
    )
    st = c.selectall(ss)
...
def pla_view_company() -> str:
    if session["logout"] == "1":
        c = conn()
        co = (
            "select * from company where plcmnt_cell_lid='"
            + str(session["pla_id"])
            + "'"
        )
        ce = c.selectall(co)
...
def edit_company(cmpid: int) -> str:
    c = conn()
    ca = "select * from company where comp_id='" + str(cmpid) + "'"
    cp = c.selectone(ca)
    return render_template("Placement cell/pla_edit_company.html", data=cp)


@app.route("/update_company", methods=["post"])
def update_company() -> str:
    comp = request.form["comp"]
    phone = request.form["contact"]
    pla = request.form["place"]
    pos = request.form["post"]
...
def delete_company(comid: int) -> str:
    c = conn()
    ca = "delete from company where comp_id='" + str(comid) + "'"
    c.nonreturn(ca)
    return pla_view_company()


@app.route("/pla_view_vacancy")
def pla_view_vacancy() -> str:
    if session["logout"] == "1":
        c = conn()
        ce = (
            "select login_id,comp_name from company where plcmnt_cell_lid='"
            + str(session["pla_id"])
            + "'"
        )
        ca = c.selectall(ce)
        return render_template(
            "Placement cell/pla_view_vacancies.html", data=ca
        )
    else:
        return login()
...
def pla_vacancy() -> str:
    ...
    ca = c.selectall(ce)
    return render_template(
        "Placement cell/pla_view_vacancies.html", data2=vy, data=ca
    )


@app.route("/pla_view_shortlists/<vid>")
def pla_view_shortlists(vid: int) -> str:
    if session["logout"] == "1":
        c = conn()
        ce = (
            "select student.*,college.col_name,course.c_name from student,college,course,shortlist where shortlist.s_id=student.login_id and student.col_id=college.login_id and course.c_id=student.c_id and shortlist.v_id='"
            + str(vid)
            + "'"
        )
        ca = c.selectall(ce)
        return render_template(
            "Placement cell/pla_view_shortlist.html", data2=ca
        )
    else:
        return login()
...
def viewmore_shortlist(sid: int) -> str:
    c = conn()
    sh = (
        "select student.*,college.col_name,course.c_name,department.d_name from student,college,course,department where department.d_id=student.d_id and student.col_id=college.login_id and course.c_id=student.c_id and student.s_id='"
        + str(sid)
        + "'"
    )
    st = c.selectone(sh)
    return render_template(
        "Placement cell/pla_viewmore_shortlist.html", data=st
    )


@app.route("/std_view_profile")
def std_viewprofile() -> str:
    if session["logout"] == "1":
        c = conn()
        st = (
            "select student.*,college.col_name,course.c_name from student,college,course where student.login_id='"
            + session["std_id"]
            + "' and college.col_id=student.col_id and course.c_id=student.c_id"
        )
        tn = c.selectall(st)
...
def std_view_internal() -> str:
    c = conn()
    sem = request.form["sem"]
    it = (
        "select subject.sub_code,subject.sub,internal.internals from student,subject,internal where student.login_id='"
        + session["std_id"]
        + "' and subject.sem='"
        + sem
        + "' and student.login_id=internal.s_id and subject.sub_id=internal.sub_id"
    )
    nt = c.selectall(it)
...
def std_view_external() -> str:
    sem = request.form["sem"]
    c = conn()
    it = (
        "select subject.sub_code,subject.sub,external.mark from subject,external where external.s_id='"
        + session["std_id"]
        + "' and subject.sem='"
        + sem
        + "' and subject.sub_id=external.sub_id"
...
def std_send_complaint() -> str:
    if session["logout"] == "1":
        c = conn()
        st = (
            "select s_id from student where login_id='"
            + session["std_id"]
            + "'"
        )
        sa = c.selectone(st)
...
def std_complaint() -> str:
    c = conn()
    cmp = request.form["complaint"]
    se = (
        "insert into complaint values(null,'"
        + session["std_id"]
        + "','"
        + cmp
...
def std_view_reply() -> str:
    if session["logout"] == "1":
        c = conn()
        re = (
            "select complaint.* from complaint where complaint.s_id='"
            + session["std_id"]
            + "'"
        )
        ry = c.selectall(re)
...
def std_delete_reply(id: int) -> str:
    c = conn()
    dt = "delete from complaint where cm_id='" + str(id) + "'"
    c.nonreturn(dt)
    return std_view_reply()


@app.route("/std_view_vacancy")
def std_view_vacancy() -> str:
    if session["logout"] == "1":
        c = conn()
...
def std_vacancy() -> str:
    ...
    co = request.form["company"]
    va = (
        "select * from vacancy where comp_id='"
        + co
        + "' and v_id not in (select v_id from requests where s_id='"
        + session["std_id"]
        + "')"
    )
    vy = c.selectall(va)
...
def std_request_add(id: int) -> str:
    c = conn()
    vy = (
...
def std_shortlist() -> str:
    c = conn()
    co = request.form["company"]
    sh = (
        "select company.comp_name,vacancy.post from company,vacancy,shortlist where company.login_id='"
        + co
        + "' and vacancy.comp_id=company.login_id and vacancy.v_id=shortlist.v_id and shortlist.s_id='"
        + session["std_id"]
        + "'"
    )
    sl = c.selectall(sh)
    ...
    co = c.selectall(cm)
    return render_template(
        "Student/std_view_shortlist.html", data2=sl, data=co
    )


@app.route("/cmp_add_vacancy")
def cmp_add_vacancy() -> str:
    if session["logout"] == "1":
...
def cmp_view_vacancy() -> str:
    if session["logout"] == "1":
        c = conn()
        cv = (
            "select * from vacancy where vacancy.comp_id='"
            + session["cmp_id"]
            + "'"
        )
        vc = c.selectall(cv)
...
def cmp_edit_vacancy(vid: int) -> str:
    c = conn()
    vy = "select * from vacancy where v_id='" + str(vid) + "'"
    va = c.selectone(vy)
    return render_template("Company/cmp_edit_vacancy.html", data=va)


@app.route("/cmp_update_vacancy", methods=["post"])
def cmp_update_vacancy() -> str:
    c = conn()
    vid = request.form["vid"]
    po = request.form["post"]
    va = request.form["vacancy"]
    ue = (
        "update vacancy set post='"
        + po
        + "',no_vacancy='"
        + va
        + "' where v_id='"
...
def cmp_delete_vacancy(vid: int) -> str:
    c = conn()
    dt = "delete from vacancy where v_id='" + str(vid) + "'"
    c.nonreturn(dt)
    return cmp_view_vacancy()


@app.route("/cmp_view_vac_reqst/<vid>")
def cmp_view_vac_reqst(vid: int) -> str:
    c = conn()
    dt = (
        "select requests.r_id,student.s_name,student.photo,college.col_name,course.c_name,student.sem,student.email,student.phone,requests.date from requests,student,college,course where requests.s_id=student.login_id and student.col_id=college.login_id and student.c_id=course.c_id  and requests.v_id='"
        + str(vid)
...
def cmp_approve_reqst(rid: int) -> str:
    c = conn()
    dt = (
        "update requests set status='not-selected' where r_id='"
        + str(rid)
        + "'"
    )
    c.nonreturn(dt)
...
def cmp_reject_reqst(rid: int) -> str:
    c = conn()
    dt = "update requests set status='rejected' where r_id='" + str(rid) + "'"
    c.nonreturn(dt)
    vid = session["vid"]
    return cmp_view_vac_reqst(vid)
...
def cmp_add_shortlist() -> str:
    if session["logout"] == "1":
        ...
        res = c.selectall(qr)
        return render_template(
            "Company/cmp_shortlist_management.html", data=res
        )
    else:
        return login()
...
def cmp_add_shortlist_post() -> str:
    ...
    if btn == "SEARCH":
        ...
        qry = c.selectall(
            "select requests.r_id,student.s_name,student.photo,college.col_name,course.c_name,student.sem,student.email,student.phone,requests.date from requests,student,college,course where requests.s_id=student.login_id and student.col_id=college.login_id and student.c_id=course.c_id  and requests.v_id='"
            + vid
            + "' and requests.status='not-selected'"
        )
        qr = (
            "select * from vacancy where comp_id='"
            + str(session["cmp_id"])
        ...
        res = c.selectall(qr)
        return render_template(
            "Company/cmp_shortlist_management.html", data=res, data2=qry
        )
    elif btn == "CREATE SHORTLIST":
        ids = request.form.getlist("idd")
        for single_id in ids:
            c.nonreturn(
                "update requests set status='approved' where r_id='"
                + str(single_id)
                + "'"
            )
            qry2 = c.selectone(
                "select v_id,s_id from requests where r_id='"
                + str(single_id)
                + "'"
            )
            c.nonreturn(
                "insert into shortlist(s_id,v_id) values('"
                + str(qry2[1])
                + "','"
                + str(qry2[0])
                + "')"
        ...
        # Handle the case when btn doesn't match any known value
        return "Invalid button clicked"


@app.route("/cmp_view_shotlist")
def cmp_view_shotlist() -> str:
    if session["logout"] == "1":
        c = conn()
        qr = (
            "select * from vacancy where comp_id='"
...
def cmp_view_shortlist_post() -> str:
    c = conn()
    vid = request.form["vcy"]
    qry = c.selectall(
        "select requests.r_id,student.s_name,student.photo,college.col_name,course.c_name,student.sem,student.email,student.phone,requests.date from requests,student,college,course where requests.s_id=student.login_id and student.col_id=college.login_id and student.c_id=course.c_id  and requests.v_id='"
    ...
    res = c.selectall(qr)
    return render_template(
        "Company/cmp_view_shortlist.html", data=res, data2=qry
    )


@app.route("/cmp_remove_shortlist/<rid>")
def cmp_remove_shortlist(rid: int) -> str:
    c = conn()
    c.nonreturn(
        "update requests set status='not-selected' where r_id='"
        + str(rid)
    ...
    qry2 = c.selectone(
        "select v_id,s_id from requests where r_id='" + str(rid) + "'"
    )
    c.nonreturn(
        "delete from shortlist where s_id='"
        + str(qry2[1])
        + "' and v_id='"
        + str(qry2[0])
        + "'"
...
def cmp_delete_ceo(cid: int) -> str:
    c = conn()
    dt = "delete from ceo where ceo_id='" + str(cid) + "'"
    c.nonreturn(dt)
    return cmp_view_ceo()
...
def verify() -> Response:
    ...
    if res is not None:
        ...
        img_url = r"D:\web\SecureID" + res[0]  # type: ignore
        ...
        if similarity["isIdentical"] is False:
    ...
    if request.form is not None:
        if "pp" in request.form:
            if request.form["pp"] != "":
                ...
                fh = open(
                    "D:\\Aproject\\NEWS PORTAL\\Web\\SecureID\\SecureID\\static\\stud_img\\"
                    + dd
                    + ".jpg",
                    "wb",
                )
...
def andsentfeedback() -> Response:
    cmp = request.form["fdbck"]
    uid = request.form["lid"]
    d = conn()
    q = (
        "INSERT INTO `feedback` (`feedback`,`date`,`studentid`) VALUES ('"
        + cmp
        + "',CURDATE(),'"
        + uid
        + "')"
...
def andsentcomplaint() -> Response:
    cmp = request.form["cmp"]
    uid = request.form["uid"]
    d = conn()
    q = (
        "INSERT INTO complaint (`s_id`,`cmplaint`,`reply`,`date`,`status`) VALUES ('"
        + uid
        + "','"
        + cmp
        + "','pending',CURDATE(),'pending')"
    )
    d.nonreturn(q)
    return jsonify(status="ok")
...
def andsentcommmentdiscussion() -> Response:
    cmp = request.form["cmp"]
    uid = request.form["uid"]
    did = request.form["did"]
    d = conn()
    q = (
        "insert into `comments` (`disid`,`comment`,`cdate`,`studentid`) values ('"
        + did
        + "','"
        + cmp
        + "',curdate(),'"
...
def and_view_profiles() -> Response:
    lid = request.form["lid"]
    qry = "SELECT * FROM student WHERE login_id='" + lid + "'"
    c = conn()
    res = c.selectone(qry)
...
def and_ratings() -> Response:
    lid = request.form["lid"]
    rating = request.form["rating"]
    d = conn()
    d.nonreturn(
        "INSERT INTO rating (`rating`,`studentid`,rdate) VALUES ('"
        + rating
        + "','"
        + lid
        + "',CURDATE())"
...
def and_newdiscussion() -> Response:
    lid = request.form["lid"]
    msg = request.form["msg"]
    d = conn()
    d.nonreturn(
        "INSERT INTO `discussion` (`studentid`,`message`,`msgdate`) VALUES ('"
        + lid
        + "','"
        + msg
        + "',curdate())"
...
def and_viewalldiscusion() -> Response:
    d = conn()
...
def clg_discussion() -> str:
    d = conn()
    qry = (
        "SELECT discussion.*,student.* FROM `discussion`,`student` WHERE student.`login_id`=`discussion`.`studentid` and student.col_id='"
        + session["clg_id"]
        + "'"
    )
    res = d.selectall(qry)
...
def clg_view_comments(id: int) -> str:
    d = conn()
    qry = (
        "SELECT `comments`.*,`discussion`.*,`student`.* FROM `comments`,`discussion`,`student` WHERE `comments`.`disid`=`discussion`.`discussionid` AND `comments`.`studentid`=`student`.`login_id` AND `discussion`.`discussionid`='"
        + str(id)
        + "'"
    )
    res = d.selectall(qry)
...
def clg_view_articles() -> str:
    d = conn()
    qry = (
        "SELECT `article`.*,`student`.* FROM `article`,`student` WHERE `article`.`student_id`=`student`.`login_id` AND student.`col_id`='"
        + session["clg_id"]
        + "'"
    )
    res = d.selectall(qry)
...
def and_vieallcommentsbydid() -> Response:
    did = request.form["did"]
    db = conn()
    qry = (
        "SELECT `comments`.*,`student`.*  FROM comments,student WHERE `comments`.`studentid`=`student`.`login_id` AND `comments`.`disid`='"
        + did
        + "'"
    )
    res = db.jsonsel(qry)
...
def and_Viewsubjects() -> Response:
    d = conn()
    lid = request.form["uid"]
    qry = (
        "SELECT * FROM `subject` WHERE `c_id` IN (SELECT `c_id` FROM `student` WHERE `login_id`='"
        + lid
        + "')"
    )
    res = d.jsonsel(qry)
...
def and_viewmaterials() -> Response:
...
def and_viewallarticles() -> Response:
...
def and_deletearticle() -> Response:
    id = request.form["aid"]
    d = conn()
...
def and_addcmment_art() -> Response:
    aid = request.form["aid"]
    comment = request.form["comment"]
    sid = request.form["uid"]
    qry = (
        "INSERT INTO `comments_art`(`aid`,`comment`,`cdate`,`studentid`) VALUES ('"
        + aid
        + "','"
        + comment
        + "',CURDATE(),'"
...
def and_vieallcomments_article_byaid() -> Response:
    did = request.form["did"]
    db = conn()
    qry = (
        "SELECT `comments_art`.*,`student`.*  FROM comments_art,student WHERE `comments_art`.`studentid`=`student`.`login_id` AND `comments_art`.`aid`='"
        + did
        + "'"
    )
    res = db.jsonsel(qry)
...
def admin_viewnotification() -> str:
    d = conn()
...
def delete_alert(aid: int) -> str:
    d = conn()
    qry = "delete from alert where alert_id='" + str(aid) + "'"
    d.nonreturn(qry)
...
def and_newarticle() -> Response:
    ...
    fh = open(
        "D:\\Aproject\\NEWS PORTAL\\Web\\SecureID\\SecureID\\static\\article\\"
        + str(timestr)
        + ".jpg",
        "wb",
    )
    ...
    d = conn()
    d.nonreturn(
        "INSERT INTO `article` (`student_id`,`file`,`date`,`description`) VALUES ('"
        + lid
        + "','"
        + path
        + "',CURDATE(),'"
